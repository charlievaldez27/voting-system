            ...
            if(isset($_POST['vote'])) {
                ...
                // $sql = "UPDATE votes set fullname = 'fullname + 1' ,schoolyear = '{$schoolyear[$i]}', position = '{$position[$i]}', candidate_id = '{$candidate_id[$i]}', voters_id = '{$voters_id[$i]}' where schoolyear = '{$schoolyear[$i]}', position = '{$position[$i]}', candidate_id = '{$candidate_id[$i]}', voters_id = '{$voters_id[$i]}' ";
                     ...
                     if($i == $count){
                         ...
                         $sql = "UPDATE status set schoolyear = '$year', Level = '$level', vStatus = 'voted', voters_id = '$votersid', Status = '$Status' where voters_id = '$votersid' "; 
                         $db->query($sql);


                        $_SESSION['message'] = "Vote successful  ";
                        $_SESSION['msg_type'] = "success";
                        header('location: votingsection.php?year='.$_GET['year']);
...
$(document).ready(function(){
 ...
 -->
  ...
  if(multiSelect.selectedOptions[i] ==true){
               alert(multiSelect.selectedOptions[i].value);
           }
}
    }
    </script>
...
<div class="container">
     <h4 style="text-align: center;"> SSC Election for Tertiary <br> School Year: <?php echo $schoolyear; ?> </h4>
     ...
     <div class="col-md-6 col-md-offset-3 mx-auto" style="max-height: 360px; overflow-y: scroll; border: 1px solid #ccc; border-radius: 5px; margin-left: 20px;  ">
        <?php  include("admin/class/Result.php");
                    ...
                    <p class="help-block"><b><?php echo $rowPos['position']; ?></b></p>
                        ...
                        <?php if($rtnReadNominee) { ?>
                            <div class="form-group" >
                                <select name="nominee[]" id="list" class="form-control">
...
<!-- For representatives -->
 ...
 <?php
                  ...
                  <?php
                    ...
                    $voter_id=$_SESSION['ID'];
                    $validateVote = new Voting();
                    $rtnValVote = $validateVote->VALIDATE_VOTE($schoolyear, $rowPos['position']);
                    ?> 
               <!--  </form> --><hr /style="background: #ffc107">
