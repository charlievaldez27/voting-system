...
$sql1 = ("SELECT * from tbl_nominees where position = '$position' and partylist = '$partylist' and Level = '$Level' and schoolyear = '$schoolyear' and id != '$id'");
		$result1 = mysqli_query($mysqli,$sql1);
		 if (mysqli_num_rows($result1) !=0) {
			header('location: addcandidates.php?position already exist in this partylist..');
			$_SESSION['message'] = "Position already taken!";
			$_SESSION['msg_type'] = "danger";
			exit();
		}
...
// 		values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
// 		$stmt->bind_param("ssssisssssssss",$schoolyear,$Level,$partylist,$position,$studentid,$Firstname,$Lastname,$MI,$Suffix,$course,$yearlevel,$CanProfile,$Status,$image);
// 		$stmt->execute();
// 		//echo "Candidate Added Successfully...";
// 		$stmt->close();
// 		$mysqli->close();
				...
				<div class="row no-gutters">
					...
					<div class="col-sm-6 no-gutters" id="createform">
						...
						<form class="stu-container" action="" method="POST" enctype="multipart/form-data">
							...
							<h1> add candidates</h1><br><br>

							<span class="asterisk_input">  </span><label class="info">Choose School Year<br>
								<select name = "schoolyear" id="syList" class ="candidateinfo" required>
							...
							<span class="asterisk_input">  </span><label class="info">Choose Partylist<br>
								<select name = "partylist" id="partylist" class ="candidateinfo action" placeholder ="Choose Partylist" required>
									<!-- <option disabled selected>Choose Year Level</option>  -->
							<?php
								if($update == true):
							?>
									...
									<option>Choose Partylist</option>
							<?php endif; ?>
							...
							<br><span class="asterisk_input">  </span><label class="info">Choose Position<br>
						...
						<?php
              			...
              			<?php } ?>
							...
							<a type="submit" class="btn btn  " style=" border: 1px solid #17a2b8; margin:20px;border-radius:5px ;  width: 115px;" href="addcandidates.php" >Cancel</a>
							<?php else: ?>
					...
					<div class="col-sm-6 no-gutters " id="createform">
						...
						<form class="stu-container" action="" method="POST">
							<h1> all candidates </h1><br><br>

								 <div class="input-group" style="width: 400px; line-height: 40px; background: transparent; padding-top: 22px">
 							<input type="text" class="form-control form-control-sm py-2 border-right-0 " name="searched" value="<?php $searched; ?>" placeholder="search by name, partylist or position" style="width: 100px; margin-left: 50PX; margin-bottom: 10px;border: none; background-color: #f1f1f1 " ><span class="input-group-append" style="background: transparent; height: 50px;">
							...
							$results =  $mysqli->query("SELECT * from tbl_nominees where CONCAT(`studentid`, `Firstname`, `Lastname`, `position`, `year`, `schoolyear`) like '%$searched%'  order by id DESC") or die($mysqli->error);

							$results1 =  $mysqli->query("SELECT * from tbl_nominees where CONCAT(`studentid`, `Firstname`, `Lastname`, `position`, `year`, `schoolyear`) like '%$searched%'  order by id DESC") or die($mysqli->error);
							...
							$results =  $mysqli->query("SELECT * from tbl_nominees order by id DESC") or die($mysqli->error);
							...
							?>
									<div class="container1 borders" id = "focus"  >
										...
										<tbody>
											...
											<?php } ?>
												<td>
													<a href="addcandidates.php?edit=<?php echo $row['id']; ?>" style = "width: 80px" class = "btn btn-info">Edit</a>
													<a href="addcandidates.php?archive=<?php echo $row['id']; ?>?" class = "btn btn-success" onClick ="return Confirmation();">Archive</a>
													<a href="addcandidates.php?delete=<?php echo $row['id']; ?>" class = "btn btn-danger" onClick ="return Confirmation();">Delete</a>
												</td>
											</tr>
										</tbody>
										<?php endwhile; ?>
	...
	$(document).ready(function(){
		$('.action').change(function(){
			if($(this).val() != '')
			{
				...
				$.ajax({
					url:"ajaxforcandidposition.php",
					method:"POST",
					data:{action:action, query:query, query1:query1}, success:function(data){
						$('#'+result).html(data);
					}
				})
			}
